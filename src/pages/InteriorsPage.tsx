...
const tabs = interiorsTabs.map((t) => ({ key: t.key, label: t.label }));

export default function InteriorsPage() {
  const activeTab = useActiveTab("residential");
  const contentRef = useRef<HTMLDivElement>(null);
  const tab = interiorsTabs.find((t) => t.key === activeTab) || interiorsTabs[0];
  ...
  return (
    <PageLayout>
      <PageHero
        ...
        subtitle="From residential sanctuaries to commercial showpieces — our interior design team creates spaces that are beautiful, functional, and deeply personal."
      />
      <TabNav tabs={tabs} />
